...
import { Plus, Pencil, Trash2, Users, X, GripVertical, Save, ChevronDown, ChevronRight } from 'lucide-react';
        ...
        <div className="flex items-center flex-1">
          ...
          <div className="flex-1">
            <div className="flex items-center">
              ...
              <span className="ml-3 text-sm text-gray-500">{group.members.length}名</span>
            </div>
            ...
            {!isExpanded && group.members.length > 0 && (
              <div className="text-xs text-gray-500 mt-1">
                ...
                {group.members.length > 5 && ` 他${group.members.length - 5}名`}
              </div>
            )}
          </div>
        </div>
        ...
        <div className="mt-4 pl-12">
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2">
            {group.members.map(memberId => {
              const user = users.find(u => u.id === memberId);
              return user ? (
                <div key={memberId} className="text-sm px-2 py-1 bg-gray-50 rounded">
                  {user.name}
                  <span className="text-xs text-gray-500 ml-1">({user.department})</span>
                </div>
              ) : null;
            })}
          </div>
...
export default function BusinessGroupManagement() {
  ...
  const [filterType, setFilterType] = useState<'all' | 'business' | 'leave'>('all');
  ...
  const fetchGroups = async () => {
    try {
      ...
      if (error) {
        ...
        setOriginalOrder(filteredGroups.map(g => g.id));
      }
    } catch (error) {
      console.error('Error fetching groups:', error);
    } finally {
